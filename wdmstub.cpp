...
{ // stub this function
KdPrint((DRIVERNAME " - Stubbing %sn", names[i]));
...
KdPrint((DRIVERNAME " - %s already implemented - not stubbing itn", names[i]));
} // for each possible stub
...
KdPrint((DRIVERNAME " - Stubbing all remove lock functionsn"));
...
KdPrint((DRIVERNAME " - Not stubbing remove lock functionsn"));
...
KdPrint((DRIVERNAME " - Stubbing %sn", critnames[i]));
...
KdPrint((DRIVERNAME " -- Unable to define export tablen"));
...
{
KdPrint((DRIVERNAME " - Can't locate driver object %ws - %Xn", objname, status));
return status;
}
...
if (!NT_SUCCESS(status))
{
KdPrint((DRIVERNAME " - ZwOpenKey(%ws) failed - %Xn", ServiceKey->Buffer, status));
return status;
}
...
if (!NT_SUCCESS(status))
{
KdPrint((DRIVERNAME " - ZwQueryValueKey failed - %Xn", status));
return status;
}
...
if (!hmod)
{
KdPrint((DRIVERNAME " - Can't get module handle to %sn", modname));
