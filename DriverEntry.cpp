...
KdPrint((DRIVERNAME " - IoCreateDevice failed - %Xn", status));
return status;
} // can't create device object
...
if (!ldo)
{ // can't attach device
KdPrint((DRIVERNAME " - IoAttachDeviceToDeviceStack failedn"));
...
{ // defer to VxD
KdPrint((DRIVERNAME " - Deferring to WDMSTUB.VXD version %d.%2.2dn", ddb->DDB_Dev_Major_Version, ddb->DDB_Dev_Minor_Version));
result = FALSE;
} // defer to VxD
else
KdPrint((DRIVERNAME " - Overriding WDMSTUB.VXD version %d.%2.2dn", ddb->DDB_Dev_Major_Version, ddb->DDB_Dev_Minor_Version));
} // VxD version loaded
...
if (!Irp)
{
KdPrint((DRIVERNAME " - Unable to allocate IRP for version queryn"));
result = FALSE;
}
else
{ // send IRP
...
KdPrint((DRIVERNAME " - Deferring to WDMSTUB clone version %d.%2.2dn", version >> 16, version & 0xFFFF));
result = FALSE;
} // found higher version
else
KdPrint((DRIVERNAME " - Overriding WDMSTUB clone version %d.%2.2dn", version >> 16, version & 0xFFFF));
} // check version #
else
KdPrint((DRIVERNAME " - IOCTL_INTERNAL_WDMSTUB_GET_VERSION failed to %ws - %Xn", devname, status));
} // send IRP
...
if (avail == 0xFFFFFFFF)
{
KdPrint((DRIVERNAME " - More than 100 WDMSTUB clones are loaded in this system!n"));
return FALSE;
}
